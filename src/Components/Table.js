...
function EnhancedTableHead(props) {
  const { order, orderBy, onRequestSort, headlines } = props;
...
export default function BasicTable(props) {
  ...
  if (rowsPerPage > 0) {
    rowsToRender = searchedRows.slice(
      page * rowsPerPage,
      ...
      >
        <Table stickyHeader sx={{ minWidth: 650 }} aria-label="simple table">
          ...
          <TableBody>
            ...
            {stableSort(rowsToRender, getComparator(order, orderBy)).map(
              (row) => {
                let out = [];
                for (let i = 0; i < renderFunctions.length; i++) {
                  out.push(
                    <TableCell
                      component="th"
                      scope="row"
                      sx={props.columnStyle && props.columnStyle[i]}
                    >
