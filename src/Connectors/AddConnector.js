...
function APIChoiceField(props) {
  ...
  props.field.depends_on &&
    props.field.depends_on.forEach((dependencyFieldName) =>
      dependencyValues.push(props.formData[dependencyFieldName])
    );
  ...
  useEffect(() => {
    if (isReady && !APIChoices) {
      ...
      props.field.depends_on &&
        props.field.depends_on.forEach(
          (dependencyFieldName) =>
            (url = url.replace(
              `{${dependencyFieldName}}`,
              props.formData[dependencyFieldName]
            ))
        );
  ...
  return (
    <Grid container justifyContent="center">
      ...
      <Grid item xs={1} alignSelf={"center"}>
        <IconButton onClick={() => setAPIChoices(null)}>
          <RefreshIcon />
        </IconButton>
      </Grid>
    </Grid>
...
export const AddConnector = () => {
  ...
  const connectorConfig = find(
    meta.fullMeta.connectors,
    (connector) => connector.name === connectorType
  );
  const fields = connectorConfig.fields;
  const widgets = connectorConfig.extra_widgets
    ? connectorConfig.extra_widgets
    : [];
  ...
  return (
    <ClippedDrawer>
      ...
      <Paper sx={{ width: "96%", overflow: "hidden", mx: "2%" }}>
        ...
        <Box component="form" py={3} px={3} onSubmit={handleSubmit}>
          <Grid container direction="column" spacing={5}>
            <Grid item textAlign={"left"}>
              <Grid container direction="row">
                ...
                {widgets.length > 0 && (
                  <Grid container xs={2} alignContent="center" marginLeft={2}>
                    <Grid item>{extraWidgetsToRender}</Grid>
                  </Grid>
                )}
              </Grid>
            </Grid>
